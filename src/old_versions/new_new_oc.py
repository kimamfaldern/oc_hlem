...
def mine_hle(df, min_time, time_segment, curr_seg_count, objects, object_handling, following_relations):
    ...
    for case in cases:
        relations = following_relations[case].copy()
        predecessor = relations['pred'].copy()
        successor = relations['succ'].copy()
        case_df = df.copy()
        case_df = case_df[case_df['ocel:process:execution']==case]
        ...
        for index, row in case_df.iterrows():
            ...
            current_pred = predecessor[current_event].copy()
            current_succ = successor[current_event].copy()
            ...
            soo = []
            for i, r in case_df.iterrows():
                ceid = r['ocel:eid']
                if ceid in current_succ:
                    so_i = r['all objects'].copy()
                    soo.append(so_i)
            ...
            if(current_succ != []):
                #check for split
                if (len(current_succ)>1):
                    #add high level event for split to log
                ...
                elif (len(current_succ)==1):
                    co = row['ocel:type:employees'].copy()
                    for c in co:
                        #add high level event of object (type) leave to log
                        event.append('object (type) ' + str(c) + ' leaves the process in activity ' + str(row['ocel:activity']))
                        ...
                        time_frame.append(curr_seg_count)
                        qcid.append(case)
                        related_object.append(c)
            else:
                co = row['all objects'].copy()
                for c in co:
                    #add high level event of object (type) leave to log
                    event.append('object (type) ' + str(c) + ' leaves the process in activity ' + str(row['ocel:activity']))
                ...
                if len(current_pred)>1:
                    ...
                    for i in late_index:
                        ...
                        if lagging_o in objects:
                            ...
                            qcid.append(case)
                            related_object.append(lagging_o)
                    #add high level event of merge to log
                    event.append('object (type) merge in activity ' + str(row['ocel:activity']) + ' with objects ' + str(row['all objects']))
                    event_type.append('j')
                    frequency.append(len(row['all objects']))
                    #TODO: check times
                    start_time.append(min_time + (curr_seg_count-1)*time_segment)
                    ...
                    qcid.append(case)
                    related_object.append(row['all objects']) 
                elif (len(current_pred)==1):
                    #check for join
                    co = row['all objects'].copy()
                    for pi, pr in pred_df.iterrows():
                        po = pr['all objects'].copy()
                        for p in po:
                            if p in co:
                                co.remove(p)
                    if co != []:
                        for c in co:
                            ...
                            event.append('object (type) ' + str(c) + ' joins the process in activity ' + str(row['ocel:activity']))
    ...
    curr_path = os.getcwd()
    parent_path = os.path.dirname(curr_path)
    save_name = parent_path + '/hl_logs_order-management2/hl_log' + str(curr_seg_count) + '.csv'
