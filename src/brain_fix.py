...
def main():
    ...
    for index, row in df.iterrows():
        if row['ocel:activity'] == 'confirm order' or row['ocel:activity'] == 'pay order' or row['ocel:activity'] == 'payment reminder':
            ...
            new_products.append(row['ocel:type:products'])
...
def mine_hle(df, min_time, time_segment, curr_seg_count, objects, object_handling, following_relations):
        ...
        for index, row in case_df.iterrows():
            ...
            if(current_succ != []):
                ...
                if (len(current_succ)>1):
                    ...
                    for si, sr in succ_df.iterrows():
                        so = succ_df['all objects'].copy()
                        for inst in so:
                            if inst in co:
                                co.remove(inst)
                    for c in co:
    ...
    curr_path = os.getcwd()
    parent_path = os.path.dirname(curr_path)
    save_name = parent_path + '/hl_logs_brain_fix/hl_log' + str(curr_seg_count) + '.csv'
